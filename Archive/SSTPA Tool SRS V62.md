SSTPA Tool - Software Requirements Specification (SRS) Version: 0.5.9.1 Date: May 15, 2026.
...
Core System Data is the data concerning the System the team of Users is creating.  This data set will be hierarchical in structure with a (:Capability) node at its root.  (:Capability) consist of administrative details on the project and it can parent User (:Sandbox) and (:System) nodes.  Only an Admin can create, or edit a (:Capability) but a User may relate a (:System) to it.  (:Sandbox) may be created and destroyed by Users at any time and are intended for tutorials, hypothetical designs and experiments.  In any case, the User may clone elements of the Sandbox into the canonical System model at any time.
...
Product Data will consist of data on the SSTPA Tool itself.  The Root Node SHALL be (:Product) with properties to include:
version number
build number
SSTPA Tools license
Release date
...
* Core System Data: every SoI sub-graph, the (:Capability) root, and
...
* The (:Capability) maps to a root package.
...
|---|---|---|---|
| (:Capability) | package | #capability | MissionAction/MissionMeans/MissionContribution as metadata attributes; owned Requirements as requirement usages. |
...
The default GUI style SHALL be the `Default_Style.css` theme and SHALL visually align with the SSTPA Tools banner logo.

The default GUI style SHALL implement a professional technical visual identity named **Technical Art Nouveau Control Room**.

The Technical Art Nouveau Control Room style SHALL combine:

* systems-engineering readability,
* precision drafting motifs,
* restrained translucent layering,
* warm ivory canvas surfaces,
* deep desaturated navy text and linework,
* restrained Art Nouveau industrial ornamentation in borders, separators, panel frames, and corner details.

The GUI SHALL prioritize readability, information density, analytical clarity, and long-session comfort over decorative visual effects.

Art Nouveau ornamentation SHALL be limited to structural UI elements such as borders, panel frames, section dividers, drawer edges, button treatments, graph frames, and branding areas.

Art Nouveau ornamentation SHALL NOT interfere with legibility, data interpretation, keyboard navigation, accessibility, or graph visualization.

Canvas areas SHALL use a warm cream-white color visually aligned with the SSTPA Tools banner logo.

Primary text SHALL use a deep desaturated navy visually aligned with the SSTPA Tools banner logo text.
...
The default font family SHALL be:

* **Source Sans 3** for primary UI text,
* **Cormorant SC** for branding, major headings, and limited display typography,
* **JetBrains Mono** for HIDs, UUIDs, backend status, ports, technical identifiers, and code-like values.

The GUI SHALL NOT require proprietary fonts for correct visual presentation.
...
The `Defaultsstpa-default.css` file SHALL be the single default UI stylesheet used by the GUI and by all Add-on Tool shells.
...
`Default_Style.css` SHALL implement the Technical Art Nouveau Control Room style.

The style foundation SHALL use warm ivory canvas surfaces, deep desaturated navy typography, engraved linework, restrained double-line borders, drafting-grid background cues, and subtle translucent layering.

The default palette SHALL include, at minimum:

* warm cream-white canvas background,
* ivory panel surface,
* deep desaturated navy primary text,
* muted steel blue secondary linework,
* restrained brass accent for selected or active states,
* muted oxblood for destructive actions.

The default style SHOULD use translucent surfaces selectively for panels, drawers, dialogs, and add-on tool windows.

Translucent effects SHALL preserve text contrast and SHALL NOT reduce readability.

The default style MAY use CSS `backdrop-filter` for restrained frosted-surface effects, provided fallback behavior remains readable where `backdrop-filter` is unsupported.

The default style SHALL avoid excessive neon glow, cyberpunk styling, aggressive tilt-card effects, high-frequency background animation, and visually distracting motion.

The default style MAY use subtle animated transitions for expand/collapse behavior, drawer movement, selection highlight, and graph recentering.

Motion SHALL be smooth, restrained, and compatible with `prefers-reduced-motion` accessibility behavior.
...
If there is no current System of Interest (SoI) selected, the panel SHALL present  (:Capability) and display the Root at top center "Select a System of Interest".
...
#### 6.5.1.17 Visual Encoding Requirements (No Icons Policy)
...
•	Tier 2 → (:Requirement) in a sub-graph whose (:System) is a direct child of an (:Component) in a sub-graph whose (:System) is a direct child of (:Capability)
